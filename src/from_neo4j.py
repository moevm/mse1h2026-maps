...
def from_neo4j(
    ...
    * the color of nodes will be set based on their label, unless there are more than 12 unique labels.
    ...
    if isinstance(data, Result):
        ...
        raise ValueError(
            f"Invalid input type `{type(data)}`. Expected `neo4j.Graph`, `neo4j.Result` or `neo4j.Driver`"
        )
...
viz_graph = from_neo4j(driver)

print(f"Узлов: {len(viz_graph.nodes)}")
print(f"Связей: {len(viz_graph.relationships)}")
...
with open("nvl_data.json", "w", encoding="utf-8") as f:
    json.dump(nvl_data, f, ensure_ascii=False, indent=2, default=str)
driver.close()
