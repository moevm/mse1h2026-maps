...
def create_nodes(tx, nodes, query):
    ...
    for node in nodes:
        try:
            ...
            raise RuntimeError(
                f"Не удалось создать узел {node.get('uid', '?')}: {e}"
            ) from e
    ...
    for rel in relationships:
        try:
            ...
            if "node not found" in str(e).lower():
                raise ValueError(
                    f"Узел не найден для связи: {rel.get('from_uid', '?')} → {rel.get('to_uid', '?')}. Ошибка: {e}"
                ) from e
            else:
            ...
            raise RuntimeError(
                f"Ошибка при создании связи {rel.get('from_uid', '?')} → {rel.get('to_uid', '?')}: {e}"
            ) from e
...
def set_to_neo4j(driver, db_name, data):
    ...
    try:
        ...
        raise ValueError(
            f"Нарушение уникальности при записи графа '{query}': {e}"
        ) from e
